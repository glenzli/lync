...
This reference covers VASM source files, workspace build configuration, build report actions, and policy diagnostics.
...
| `vasmc.yaml` | Remote dependency declarations. |
| `vasmc-lock.yaml` | Remote dependency lockfile. Commit this file. |
...
| Field | Type | Meaning |
| --- | --- | --- |
| `alias` | string | Local alias. `vasmc add` can use it when resolving remote modules. |
...
| Status | Meaning |
| --- | --- |
| `pass` | No policy diagnostics. |
| `review` | Human or AI judgment is required. |
...
| `policy_review` | entry | AI reviews diagnostics at review level. |
...
| `policy.content.prompt_override` | content | review | Text appears to override higher-priority instructions. |
| `policy.content.concealment` | content | review | Text appears to hide behavior from the user or reviewer. |
| `policy.content.secret_exfiltration` | content | review | Text appears to combine secret access with exfiltration language. |
| `policy.content.remote_execution` | content | review | Text appears to download and execute remote code. |

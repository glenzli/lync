...
`vasmc sync` reads the catalog, verifies the artifact hash, writes the artifact into `.vasmc/` or an explicit `dest`, and records `source: catalog`, `catalog`, `export`, `name`, `version`, `format`, `hash`, and the resolved artifact URL in `vasmc-lock.yaml`. After that, `@import` still resolves only through `vasm:<alias>` plus the local lockfile:

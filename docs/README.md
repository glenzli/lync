...
This directory contains generated documentation. Maintain the source files in `docs-src/`; do not edit `docs/*.md` directly.
...
2. [AI Workflow](AI-WORKFLOW.md): explains how AI editors should read `.vasmc/build-report.yaml` and execute verify, translate, tree-shake, policy, and project review actions.
3. [Protocol Reference](REFERENCE.md): lists manifest fields, imports, language blocks, build config, report actions, and policy diagnostics.
...
2. [AI 工作流](AI-WORKFLOW.md)：说明 AI 编辑器如何读取 `.vasmc/build-report.yaml`，执行 verify、translate、tree-shake、policy 和 project review。
3. [协议参考](REFERENCE.md)：完整列出 manifest、import、语言块、build config、report actions 和 policy diagnostics。

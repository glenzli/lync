...
- 除 `translate` action 明确要求写入 `targets`，或 `refresh_translation` action 明确要求检查并更新已保留目标语种段外，不要直接编辑生成的 `.md` 产物。
...
[校验标准](../../../packages/core/vasm/fragments/verify-criteria.md "@import:inline")

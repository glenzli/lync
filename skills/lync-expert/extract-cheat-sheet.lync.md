...
2. The cheat sheet MUST cover:
   ...
   - i18n blocks (`<!-- lang:xx -->`).
   - A list of all CLI commands (`lync build`, `lync add`, `lync graph`, `lync seal`, `lync sync`, `lync build --verify`, `lync build --verify-continue-on-error`, `lync build --diff`) with a one-sentence explanation for each.
